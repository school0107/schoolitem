...
public class SweepAttackListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (cooldownMap.containsKey(playerId)) {
            ...
            if ((currentTime - lastUse) < cooldownSeconds * 1000) {
                ...
                if (!lastMessageMap.containsKey(playerId) || 
                    (currentTime - lastMessageMap.get(playerId)) > 3000) {
                    lastMessageMap.put(playerId, currentTime);
    ...
    private void activateSweepAttack(Player player, double damage, int range) {
        ...
        for (int i = 0; i <= range * 2; i++) {
            ...
            player.getWorld().spawnParticle(Particle.SWEEP_ATTACK, checkPoint, 2, 0.2, 0.2, 0.2, 0);
            
            if (i % 2 == 0) {
                player.getWorld().spawnParticle(Particle.CRIT, checkPoint, 3, 0.2, 0.2, 0.2, 0.05);
            }
            ...
            for (Entity entity : player.getWorld().getNearbyEntities(checkPoint, 1.5, 1.5, 1.5)) {
                ...
                // Chỉ hiển thị 1 lần mỗi 2 giây - KHÔNG LOG
                UUID playerId = player.getUniqueId();
                long currentTime = System.currentTimeMillis();
                if (!lastMessageMap.containsKey(playerId) || 
                    (currentTime - lastMessageMap.get(playerId)) > 2000) {
                    lastMessageMap.put(playerId, currentTime);
                    player.sendMessage(ColorUtils.colorize(
        ...
        player.getWorld().spawnParticle(Particle.EXPLOSION, endLoc, 3, 0.5, 0.5, 0.5, 0.1);
